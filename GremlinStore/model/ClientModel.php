<?php
    class ClientModel {
        protected $rsa, $publicKey, $privateKey, $client;

        public function __construct() {
            include('public/rsa/Crypt/RSA_XML.php');
            require_once 'public/rsa/nusoap/lib/nusoap.php';
            $this->publicKey = "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";
            $this->privateKey = "<xml><RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent><P>2t44Cs96qiQnTPmrIWtknBQBtvEqapuTKGAlZYUL9TtUsahHFoIasT0Sh7s473NJLhlYBiY/DqJCMfBXirWi/jYF1SkaTLrHafViUT7nWdKshkKWk+F5QUWafYqKpuS0yvWAwboK8h9HDBdzBvKJToPCRfhxmR634ILReoHoi7M=</P><Q>/YmFhL3zcMZAphJn4Pk19vznhx6Efi+m1kk94wvMpMGE33FMxhrS+AOrXcTuSSlQRMpDJyK4eoZA7k4ywcy2qvVBo8kP3v9aA+tV7CSy7dax2Uy/uU/g+B5i8es7YidumD3Y3+uax96XLKk/JqDR4GYTsEoDiDrBpQvhOUDQy78=</Q><DP>Z1ggPDKFqArUJ8uH24rbAbuMp7YuIhJ9pMXpQotU/yFZt11O27jpI6FDu6VSl/Xmqwa6goTSp3VELlJe85cadFBL5r+2tF5JRKXiG5zJHqeUYPzWHtOLLp/P99FP8Mfy+hZ8Cn1vJ65PHM2BudkRrmoWdHWAJh3UEwWSJfdrN3M=</DP><DQ>4/Y4tQfN0UKQckHuYNvyZ6cxzc3kt0jFK7sjB/heAm11uSo4K4+GzTUPafpdfi7FW3S5OIEBejkoPJzXCmW0gLaQVGG+AXV7TDbXRMIil7MIqidV45ss3isEMByfgjuONxi03XEL9FXxe1iZzXOgAbIGVeXWkzCxWGL8GOxsUCU=</DQ><InverseQ>J7w9o8/ukZMRuB/KQ3LP3W73E+LFVvXtH48sFzhqwemrvqE6xLHxEsyxVuvtelxjvqENbjTEZA9JDRD0pMvQbjCYUtPLLRPNm5GucY/rRczoBHCmh0cv/8zZDqlmMZcO8ef+eH7W4j/juOkulvdz4n3iX8oio8Hg4yTkj4AWqfE=</InverseQ><D></D></RSAKeyValue></xml>";
            $this->rsa = new Crypt_RSA_XML();
            $this->client = new nusoap_client("http://10.234.222.227:8095/WebServiceLoginSoap.asmx?WSDL", true);
        }//construct

        public function registerClient($name, $card, $email, $pass){
            $this->rsa->loadKey($this->publicKey);
            $nameEncrypt = base64_encode($this->rsa->encrypt($name));
            $cardEncrypt = base64_encode($this->rsa->encrypt($card));
            $emailEncrypt = base64_encode($this->rsa->encrypt($email));
            $passEncrypt = base64_encode($this->rsa->encrypt($pass));
            $param = array('name' => $nameEncrypt, 'mail' => $emailEncrypt, 'pass' => $passEncrypt, 'card' => $cardEncrypt);
            $result = $this->client->call('RegisterClient', $param);
            return $result['RegisterClientResult'];
        }//registerClient
        
        public function loginVerify($email, $password){
            $this->rsa->loadKey($this->publicKey);
            $emailEncrypt = base64_encode($this->rsa->encrypt($email));
            $passEncrypt = base64_encode($this->rsa->encrypt($password));
            $param = array('mail' => $emailEncrypt, 'pass' => $passEncrypt);
            $result = $this->client->call('LoginClient', $param);
            return $result['LoginClientResult'];
        }//loginVerify
        
        public function updateClientData($name, $card, $email, $pass){
            $this->rsa->loadKey($this->publicKey);
            $nameEncrypt = base64_encode($this->rsa->encrypt($name));
            $cardEncrypt = base64_encode($this->rsa->encrypt($card));
            $emailEncrypt = base64_encode($this->rsa->encrypt($email));
            $passEncrypt = base64_encode($this->rsa->encrypt($pass));
            $param = array('name' => $nameEncrypt, 'mail' => $emailEncrypt, 'pass' => $passEncrypt, 'card' => $cardEncrypt);
            $result = $this->client->call('UpdateClient', $param);
            return $result['UpdateClientResult'];
        }//updateDataClient
        
    }//ClientModel
?>

